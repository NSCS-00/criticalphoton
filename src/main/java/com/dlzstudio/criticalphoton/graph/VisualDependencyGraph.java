...
public class VisualDependencyGraph {
        ...
        if (blocks != null) {
            for (long blockKey : blocks) {
                ...
                if (entities != null) {
                    ...
                    if (entities.isEmpty()) blockToEntities.remove(blockKey);
                }
            }
        }
    ...
    public void updateChunkVisibility(Level level, LevelChunk chunk, FrustumVisibility frustum) {
        ...
        int maxX = chunkPos.getMaxBlockX(), maxZ = chunkPos.getMaxBlockZ();
        ...
        for (long blockKey : blockToEntities.keySet()) {
            ...
            if (pos.getX() >= minX && pos.getX() <= maxX && pos.getZ() >= minZ && pos.getZ() <= maxZ) {
                if (isPotentiallyVisible(frustum, pos)) {
                    List<Entity> entities = blockToEntities.get(blockKey);
                    if (entities != null) entities.forEach(this::scheduleEntityRender);
                }
            }
        }
    }
    ...
    public boolean isPotentiallyVisible(FrustumVisibility frustum, BlockPos pos) {
        return frustum.isVisible(pos) || frustum.isInExtendedFrustum(pos, maxDependencyDistance);
    }
    
    public void scheduleEntityRender(Entity entity) { potentiallyVisibleEntities.add(entity); }
    public VisualAnchorConfig getAnchorConfig(Entity entity) { return entityAnchorConfigs.get(entity); }
    ...
    public Set<Entity> getPotentiallyVisibleEntities() { return Collections.unmodifiableSet(potentiallyVisibleEntities); }
    public void clearVisibilityCache() { potentiallyVisibleEntities.clear(); }
